...
async def process ():
    # instances
    prefix = 'n:'
    dh   = await model_ensure_instance('%s/dh'%prefix  , 'gfb:District_Heat' , escape('fjernfarme'))
    hx   = await model_ensure_instance('%s/hx'%prefix  , 'gfb:Heat_Exchanger', escape('district2heated'))
    tank      = await model_ensure_instance('%s/tank'%prefix     , 'gfb:Hot_Water_Tank'          , escape('varmtvand'))
    tank_temp = await model_ensure_instance('%s/tank/temp'%prefix, 'gfb:Water_Temperature_Sensor', escape('varmtvand_temperatur'))
    d_ret_flow  = await model_ensure_instance('%s/district/flow'%prefix           , 'gfb:Water_Flow_Sensor'                 , escape('district_flow'))
    d_ret_temp  = await model_ensure_instance('%s/district/return/temp'%prefix    , 'gfb:Water_Temperature_Sensor'          , escape('district_return_temp'))
    d_sup_temp  = await model_ensure_instance('%s/district/supply/temp'%prefix    , 'gfb:Water_Temperature_Sensor'          , escape('district_supply_temp'))
    d_ret_valve = await model_ensure_instance('%s/district/return/valve'%prefix   , 'gfb:Valve'                             , escape('district_return_valve'))
    d_ret_pres  = await model_ensure_instance('%s/district/return/pressure'%prefix, 'gfb:Water_Pressure_Sensor'             , escape('district_return_pressure'))
    d_sup_pres  = await model_ensure_instance('%s/district/supply/pressure'%prefix, 'gfb:Water_Pressure_Sensor'             , escape('district_supply_pressure'))
    d_dif_pres  = await model_ensure_instance('%s/district/diff/pressure'%prefix  , 'gfb:Water_Differential_Pressure_Sensor', escape('district_differential_pressure'))
    d_heat      = await model_ensure_instance('%s/district/heat'%prefix           , 'gfb:Heat_Meter'                        , escape('district_heat_meter'))
    h_sup_temp = await model_ensure_instance('%s/heated/supply/temp'%prefix, 'gfb:Water_Temperature_Sensor', escape('heated_supply_temp'))
    h_l1_sup_valve = await model_ensure_instance('%s/heated/loop1/supply/valve'%prefix, 'gfb:Valve'                   , escape('heated_loop1_supply_valve'))
    h_l1_sup_temp  = await model_ensure_instance('%s/heated/loop1/supply/temp'%prefix , 'gfb:Water_Temperature_Sensor', escape('heated_loop1_supply_temp'))
    h_l1_sup_pump  = await model_ensure_instance('%s/heated/loop1/supply/pump'%prefix , 'gfb:Pump'                    , escape('heated_loop1_supply_pump'))
    h_l1_radiator  = await model_ensure_instance('%s/heated/loop1/radiator'%prefix    , 'gfb:Radiator'                , escape('heated_loop1_radiator'))
    h_l2_sup_bvalve    = await model_ensure_instance('%s/heated/loop2/supply/bvalve'%prefix         , 'gfb:Bypass_Valve'            , escape('heated_loop2_supply_bypass_valve'))
    h_l2_sup_temp      = await model_ensure_instance('%s/heated/loop2/supply/temp'%prefix           , 'gfb:Water_Temperature_Sensor', escape('heated_loop2_supply_temp'))
    h_l2_ret_pump      = await model_ensure_instance('%s/heated/loop1/return/pump'%prefix           , 'gfb:Pump'                    , escape('heated_loop1_return_pump'))
    h_l2_ret_temp_pre  = await model_ensure_instance('%s/heated/loop2/return/prebypass/temp'%prefix , 'gfb:Water_Temperature_Sensor', escape('heated_loop2_return_prebypass_temp'))
    h_l2_ret_temp_post = await model_ensure_instance('%s/heated/loop2/return/postbypass/temp'%prefix, 'gfb:Water_Temperature_Sensor', escape('heated_loop2_return_postbypass_temp'))
    h_l2_radiator      = await model_ensure_instance('%s/heated/loop2/radiator'%prefix              , 'gfb:Radiator'                , escape('heated_loop2_radiator'))
    h_l3_ret_valve = await model_ensure_instance('%s/heated/loop3/return/valve'%prefix, 'gfb:Valve', escape('heated_loop3_return_valve'))
